...
export function Footer() {
  ...
  return (
    <>
      <section dir={direction} className="px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl bg-cedar px-6 py-14 text-center text-ivory shadow-soft sm:px-10">
          ...
          <a
            href={whatsappHref}
            className={`mt-8 inline-flex items-center justify-center gap-2 rounded-full bg-brass px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-ink transition hover:bg-ivory ${iconDirection}`}
          >
            {footer.ctaButton}
            <ArrowRight size={18} />
          </a>
      ...
      <footer dir={direction} className="border-t border-brass/20 px-4 py-8 text-center text-sm text-coffee sm:px-6 lg:px-8">
        <div className={`mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_1.4fr] ${textAlignment}`}>
          <div>
            ...
            <a href={whatsappHref} className={`mt-4 inline-flex items-center gap-2 font-bold text-henna ${iconDirection}`}>
